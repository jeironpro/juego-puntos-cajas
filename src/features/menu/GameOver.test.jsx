...
describe('GameOver', () => {
  it('shows the winner name', () => {
    render(<GameOver winnerName="TÚ" onPlayAgain={() => {}} onMenu={() => {}} />);
    expect(screen.getByRole('heading', { name: 'Gana TÚ' })).toBeInTheDocument();
  });

  it('shows a tie when there is no winner', () => {
    render(<GameOver winnerName={null} onPlayAgain={() => {}} onMenu={() => {}} />);
  ...
  it('triggers the play again and menu actions', async () => {
    ...
    const onMenu = vi.fn();
    render(<GameOver winnerName="BOT" onPlayAgain={onPlayAgain} onMenu={onMenu} />);
